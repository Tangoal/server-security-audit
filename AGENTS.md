# AGENTS.md — server-security-audit
...
1. **Collecte** une quarantaine de relevés en lecture seule (SSH, pare-feu,
   exposition publique via tunnel et reverse-proxy, ports, comptes et groupes,
...
bonne santé de l'audit : pas de mail = l'audit ne tourne plus. Un rapport
« RAS » qu'on ne lit pas coûte moins cher qu'une panne silencieuse.

**Les rapports vivent dans le dossier de travail**, donc peuvent être couverts
par n'importe quelle sauvegarde générale de ce dossier (ex: `server-backup-gdrive`,
projet compagnon), sans mécanisme de sauvegarde dédié.
...
par **liste blanche** : la configuration dynamique d'un reverse-proxy comme
Traefik n'affiche que les clés de structure et une poignée de clés sûres
(`rule`, `entrypoints`, `middlewares`…), parce que ces fichiers portent aussi
les empreintes `basicAuth` ; un tunnel type Cloudflare n'affiche que les
`hostname`/`service`, jamais son fichier de credentials. Une liste noire ne
conviendrait pas ici : elle laisse passer ce qu'on n'a pas prévu.

**Le pare-feu ne décrit pas la surface publique.** Sur une machine derrière un
tunnel, `ufw` peut légitimement tout refuser en entrée pendant que vingt
hostnames sont publiés vers un reverse-proxy. D'où la section du rapport qui
croise les hostnames du tunnel avec les routes du reverse-proxy et repère
celles qui sont exposées sans middleware d'authentification. Sans elle,
l'audit conclurait « ✅ pare-feu cohérent » sur une machine dont toute la
surface d'attaque réelle est ailleurs.
...
- **`reports/`** — toutes les machines écrivent `reports/<date>.md`, le même
  chemin : les versionner garantit un conflit chaque semaine. Selon votre
  setup, ils peuvent déjà être couverts par une sauvegarde générale du dossier
  de travail.
...
Sur les machines qui n'ont pas à pousser de code, préférer une **clé de
déploiement en lecture seule** (clé SSH dédiée + entrée `Host` dans
`~/.ssh/config`) plutôt qu'un token de compte : un serveur compromis ne doit
pas pouvoir réécrire le code d'audit des autres.
...
# 5. Premier run complet (facultatif, sinon le timer s'en charge à l'heure prévue)
...
Fixer un horaire identique sur toutes les machines (le timer généré par
`install.sh` peut être ajusté) permet à deux rapports de la même semaine de
décrire le parc au même instant.

## Configurer les alertes email (Resend)

`RESEND_API_KEY`/`ALERT_EMAIL_TO`/`ALERT_EMAIL_FROM` sont à votre charge, rien
n'est câblé en dur :

1. Créer un compte [Resend](https://resend.com) (gratuit pour un usage bas
   volume) et générer une clé API en envoi seul.
2. Vérifier votre propre domaine d'envoi dans Resend (DKIM + SPF chez votre
   ...
   dès qu'on écrit à quelqu'un d'autre que le titulaire du compte (`403
   validation_error`), et part souvent en indésirables (domaine partagé, pas
   de DKIM/SPF au nom de l'expéditeur réel).
3. Renseigner `ALERT_EMAIL_FROM=alertes@votre-domaine` une fois le domaine
   vérifié.

Une clé en envoi seul est rejetée sur `/domains` et `GET /emails/<id>` :
impossible de vérifier le statut de remise d'un message depuis la machine —
d'où la journalisation de l'id à l'envoi plutôt qu'une vérification active.
...
- **Les couches d'images de conteneurs sont pleines de binaires SUID** (leur
  propre `su`, `passwd`…). Sans `-prune` sur les répertoires de runtime
  conteneurs (Docker, containerd, snap), ils noient les vrais SUID de l'hôte.
...
- **Chercher « token » ou « credentials » dans un chemin ne cherche pas des
  secrets.** La première version du relevé des secrets versionnés remontait des
  faux positifs (une migration nommée avec « token », un fichier de doc nommé
  avec « credentials »). Le motif porte désormais sur le **nom de fichier**
  seul, pas sur des mots-clés génériques dans le chemin.
- **Ne pas laisser `AUDIT_MODEL` vide.** Ce serait le modèle de la session
  ...
  l'audit du lendemain sans que rien ne le signale.
...
redémarrage de service. Le pipeline de sauvegarde et l'audit documentaire de
projets sont des sujets distincts, sans dépendance avec celui-ci.
